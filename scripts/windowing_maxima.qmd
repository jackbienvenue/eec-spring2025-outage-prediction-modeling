...
#####-----FUNCTION PHASE 1: READ IN DATA, MERGE GRID CELLS TO TOWNS-----##### 
    ...
    ### Calculate the distance matrix between all towns' centroids and grid cells' centroids
    distance_matrix = ct_towns_gdf['centroid'].apply(
        lambda town_centroid: grid_cells_gdf['centroid'].distance(town_centroid)
    ).T

    ### Find nearest grid cell for each town
    nearest_grid_cell_idx = distance_matrix.idxmin(axis=1)

    ### Combine nearest grid cell and town data
    nearest_grid_cells = grid_cells_gdf.loc[nearest_grid_cell_idx].reset_index(drop=True)
    
    ### Assignment of grid cells to towns
    nearest_grid_cells['town_name'] = ct_towns_gdf.loc[nearest_grid_cell_idx, 'TOWN_NAME'].values

    ### Drop repeated columns pertaining to town centroid and geometry
    ### to avoid errors later 
    town_data = ct_towns_gdf.drop(columns=['centroid', 'geometry'])
    combined_data = pd.concat([nearest_grid_cells, town_data], axis=1)

    ### Create gdf from the combined data
    joined_gc_towns = gpd.GeoDataFrame(combined_data, geometry=nearest_grid_cells['centroid'])

    ## Ensure the geometry column is correctly set (this might be needed depending on the context)
    joined_gc_towns.set_geometry('geometry', inplace=True)
