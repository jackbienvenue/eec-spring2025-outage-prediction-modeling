...
#####-----   FUNCTION PHASE 1: READ IN DATA, MERGE GRID CELLS TO TOWNS    -----#####
    ...
    '''
    NOTE: The lon and lat columns added to joined_gc_towns in this 
    section are geocodes for GRID CELL CENTROIDS, which are identifiers
    that are used in the filenames for the grid cell weather data
    '''
...
#####-----   FUNCTION PHASE 2: USE WINDOWING TO MAXIMIZE DRIVER VALUES   -----#####
    ...
    print(town_outages) # Refresher on what we have for data
    print(joined_gc_towns) ##### DELETE LATER
    ...
    town_outages['storm_start_datetime'] = pd.to_datetime(town_outages['storm_start_datetime']) # Ensure datetime format here
    ...
    def weather_data_retrieval(gc_file_name, datetime):

        '''
        This function is designed for the iteration process of taking events and 
        finding maximum values for weather data. It will be used later on in phase 2.2
        to retrieve weather data for events. 

        ARGUMENTS:
        gc_file_name - the file name for the merged grid cell which corresponds to the town that we are retrieving data for. This will be generated later in the code
        iteratively and be fed into this function.
        datetime - the time of the event
        '''

        for event in gdf.itertuples():  # Use itertuples for better performance
            event_start_time = event.start_time  # Assuming 'start_time' is a column in gdf
            event_end_time = event_start_time + pd.Timedelta(hours=48)  # 48-hour window

            # Iterate through pseudo-days (24 pseudo-days within the 48-hour window)
            max_means = {}  # Dictionary to store the max mean for each variable
            for pseudo_day in range(24):  # Loop through pseudo-days (0 to 23)
                pseudo_day_start = event_start_time + pd.Timedelta(hours=pseudo_day)
                pseudo_day_end = pseudo_day_start + pd.Timedelta(hours=24)  # 24-hour pseudo-day

                # Filter the data for this pseudo-day (within the 48-hour window)
                pseudo_day_data = gdf[(gdf['time'] >= pseudo_day_start) & (gdf['time'] < pseudo_day_end)]
                
                # Calculate the mean for each independent variable in this pseudo-day
                means = pseudo_day_data[['var1', 'var2', 'var3']].mean() 
                
                # Store the maximum means for each variable
                for var in means.index:
                    if var not in max_means or means[var] > max_means[var]:
                        max_means[var] = means[var]
            return 
    ...
    # Iterate through events to initiate the process of matching event to grid cell
    for event in event_df.iterrows:

        # Find alphabetically first town listed and remove it
        town = event_df['towns_affected'].apply(lambda x: x.split(',')[0])

        # Check if we find a match for the town name

        if town in town_outages['town'].values:
            # If we do, extract grid cell centroid 
            centroid = town_outages[town_outages['town'] == town]['centroid'].values[0]

            # Now, begin to retrieve windowed weather data:


        else:
            # If not, report that we cannot
            print("Town not found in town_outages")
