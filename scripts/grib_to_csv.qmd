...
```
    grouped = [group.set_index(['latitude', 'longitude']).reset_index() for _, group in df.groupby(['latitude', 'longitude'])]
...
```
    ...
    def export_dfs_to_csv(dfs, output_directory_path): #dfs is a list!
        ...
        for idx, group_df in enumerate(dfs):
            ...
            lat = group_df['latitude'].iloc[0]  # Get the first value (assuming all values are the same for each group)
            lon = group_df['longitude'].iloc[0]  # Same as above for longitude
            
            # Create a filename using latitude, longitude, and an index
            filename = f"lat_{lat}_lon_{lon}_time_series_weather.csv"
            
            # Ensure it's a valid filename
            filename = filename.replace('.', '_')  # Replace dots for better compatibility in filenames
