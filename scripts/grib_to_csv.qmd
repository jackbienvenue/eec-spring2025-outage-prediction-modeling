...
def grib_folder_processing(earliest_file, latest_file, input_directory, output_directory_path, hourly=False):
                    ...
                    for step in range(25):
                        hourly_data = cfgrib.open_dataset(
                            ...
                            backend_kwargs={
                                'filter_by_keys': {
                                    'typeOfLevel': 'surface',
                                    'step': step  # Step corresponding to each hour (0 to 23)
                                }
                            }
                        )
                        hourly_df = hourly_data.to_dataframe()
    ...
    for i, df in enumerate(df_list):
        ...
        df['latitude1'] = df['latitude'].round(5)  # Create latitude1 column
        df['longitude1'] = df['longitude'].round(5)  # Create longitude1 column
    ...
    #1. Isolate individual points throughout timeseries
        ...
        #1.a Let's subset our variables so we only take the ones we want:

    # List of variables you want to keep
    keep_variables = ['t2m', 'tp', 'u10', 'v10', 'latitude1', 'longitude1', 'valid_time'] #'time', 'latitude', 'longitude', 'step'

    # Select the desired columns
    df = combined_df[keep_variables] # Subset to drop extraneous columns
    ...
    for i, group_df in enumerate(grouped):
        ...
        group_df['valid_time'] = pd.to_datetime(group_df['valid_time'], format='%m/%d/%Y %I:%M:%S %p', errors='coerce')
...
# Function use:
grib_folder_processing("download_ERA5_LAND_package_2023_01.grib", "download_ERA5_LAND_package_2023_02.grib", "../data/data_CT/", "/Volumes/JB_Fortress_L3/hourly_resolution_csvs", hourly=True)

```

NOTE: THE ORIGINAL GRIB FILES SUPPORT AN HOURLY RESOLUTION, AND NOTE THE FOLLOWING STRUCTURAL COMPONENTS:
